...
const TEMPLATE_MAPPING = [
  {
    ...
    getReplacements: async (client, blockName, suffix) => {
      ...
      const violationsTable = `${blockName}_violations${suffix}`;
...
// Helper function to send batched Teams notification
async function sendBatchedTeamsNotification(teamsWebhook, dashboardUrls, username) {
  if (!teamsWebhook || dashboardUrls.length === 0) return;
  try {
    console.log(`[DEBUG] Preparing to send Teams notification with ${dashboardUrls.length} URLs`);
    const message = `New/Updated Dashboards for ${username}:\n${dashboardUrls
      .map(({ url, tableName }) => `- ${tableName}: ${url}`)
      .join('\n')}`;
    await notificationService.postDashboardLinkToTeams(teamsWebhook, message, "Batch Notification", username);
    console.log("[DEBUG] Sent batched Teams notification for", dashboardUrls.length, "dashboards");
  } catch (error) {
    console.error("[ERROR] Failed to send batched Teams notification:", error);
  }
}

// Helper function to send batched Slack notification
async function sendBatchedSlackNotification(slackToken, slackChan, dashboardUrls, username) {
  if (!slackToken || !slackChan || dashboardUrls.length === 0) return;
  try {
    console.log(`[DEBUG] Preparing to send Slack notification with ${dashboardUrls.length} URLs`);
    const message = `New/Updated Dashboards for ${username}:\n${dashboardUrls
      .map(({ url, tableName }) => `- ${tableName}: ${url}`)
      .join('\n')}`;
    await notificationService.postDashboardLinkToSlack(slackToken, slackChan, message, "Batch Notification", username);
    console.log("[DEBUG] Sent batched Slack notification for", dashboardUrls.length, "dashboards");
  } catch (error) {
    console.error("[ERROR] Failed to send batched Slack notification:", error);
  }
}
...
async function scanAndCreateDashboardsForUser(userId) {
  ...
  try {
    ...
    await sendBatchedTeamsNotification(teamsWebhook, dashboardUrls, username);
    await sendBatchedSlackNotification(slackToken, slackChan, dashboardUrls, username);
...
async function processTimingReportDb(
  ...
  try {
    ...
    const singleUrls = await processSingleTableTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey);
    const compareUrls = await processComparisonTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey);
    dashboardUrls.push(...singleUrls, ...compareUrls);
  } catch (error) {
...
async function processSingleTableTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey) {
  ...
  for (const row of tableResult.rows) {
    ...
    const existingDash = await pool.query(
      `SELECT id FROM dashboardtiming WHERE user_id = $1 AND table_name = $2`,
      [userId, tableName]
    );
    ...
    const templateConfig = TEMPLATE_MAPPING.find(({ pattern, type }) =>
      pattern?.test(tableName) && ["delay-stack"].includes(type)
    );
    ...
    try {
      ...
      dashboardUrls.push({ url: dashboardUrl, tableName });
    } catch (tableError) {
...
async function processComparisonTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey) {
  const tableResult = await client.query(`
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public'
  ...
  for (const [runKey, { cts, route, prefix }] of runMap) {
    ...
    try {
      ...
      if (existingDelayDash.rows.length > 0) {
        // Check if dashboard needs updating due to new tables
        ...
        const needsUpdate =
          (ctsTableExists && delayReplacements["{{CTS_TABLE}}"] === cts) ||
          (routeTableExists && delayReplacements["{{ROUTE_TABLE}}"] === route) ||
      ...
      dashboardUrls.push({ url: delayDashboardUrl, tableName: delayTableName });
    } catch (error) {
    ...
    try {
      ...
      if (existingSlackDash.rows.length > 0) {
        ...
        const needsUpdate =
          (ctsTableExists && slackReplacements["{{CTS_TABLE}}"] === cts) ||
          (routeTableExists && slackReplacements["{{ROUTE_TABLE}}"] === route) ||
      ...
      dashboardUrls.push({ url: slackDashboardUrl, tableName: slackTableName });
    } catch (error) {
...
async function processReportsDb(
  ...
  try {
    ...
    const singleUrls = await processSingleTableReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey);
    const compareUrls = await processComparisonReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey);
    const runCompareUrls = await processRunCompareDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey);
    dashboardUrls.push(...singleUrls, ...compareUrls, ...runCompareUrls);
  } catch (error) {
...
async function processSingleTableReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey) {
  const tableResult = await client.query(`
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public'
  ...
  // Group tables by block name for QoR dashboards
  ...
  tableResult.rows.forEach((row) => {
    ...
    if ((match = tableName.match(/^(.*)_grafana_pd(?:_csv)?$/i)) ||
        ...
        (match = tableName.match(/^(.*)_violations(?:_csv)?$/i))) {
  ...
  // Process QoR dashboards for each block
  ...
  for (const [blockName, { tables, suffix }] of blockMap) {
    ...
    try {
      ...
      if (existingDash.rows.length > 0) {
        ...
        const needsUpdate =
          (grafanaTableExists && replacements["{{RUN_TABLE}}"] === grafanaTable) ||
          (pathgroupsTableExists && replacements["{{TIME_TABLE}}"] === pathgroupsTable) ||
          (violationsTableExists && replacements["{{DRC_TABLE}}"] === violationsTable) ||
      ...
      dashboardUrls.push({ url: dashboardUrl, tableName: canonicalTableName });
    } catch (blockError) {
...
async function processComparisonReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey) {
  const tableResult = await client.query(`
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public'
...
async function processRunCompareDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey) {
  const tableResult = await client.query(`
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public'
    ...
    try {
      ...
      if (existingDash.rows.length > 0) {
        ...
        for (const { key } of requiredTables) {
          ...
          if (tableExists || tableName.startsWith("placeholder")) {
            needsUpdate = true;
            break;
          }
        }
      ...
      dashboardUrls.push({ url: dashboardUrl, tableName: `Run Comparison: ${pairKey}` });
    } catch (error) {
