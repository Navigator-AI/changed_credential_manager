  ...
  const replaceInObject = (current) => {
    for (const key in current) {
      if (typeof current[key] === "string") {
        ...
        // Handle placeholder tables gracefully
        if (
          current[key].includes("placeholder_run_table") ||
          current[key].includes("placeholder_time_table") ||
          current[key].includes("placeholder_drc_table")
        ) {
          current[key] = current[key].replace(
            /FROM\s+placeholder_(run|time|drc)_table/g,
            "FROM (SELECT 1 WHERE FALSE) AS placeholder_$1_table"
          );
        }
...
async function processTimingReportDb(
  ...
  try {
    ...
    await processSingleTableTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey, slackToken, slackChan, teamsWebhook);
    await processComparisonTimingDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey, slackToken, slackChan, teamsWebhook);
  } catch (error) {
...
async function processReportsDb(
  ...
  try {
    ...
    await processSingleTableReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey, slackToken, slackChan, teamsWebhook);
    await processComparisonReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey, slackToken, slackChan, teamsWebhook);
...
async function processSingleTableReportsDashboards(client, userId, username, dbName, dsUid, grafanaUrl, grafanaKey, slackToken, slackChan, teamsWebhook) {
  ...
  for (const [blockName, { tables, suffix }] of blockMap) {
    ...
    try {
      ...
      if (slackToken && slackChan) {
        await notificationService.postDashboardLinkToSlack(slackToken, slackChan, dashboardUrl, canonicalTableName, username);
      }

      if (teamsWebhook) {
        await notificationService.postDashboardLinkToTeams(teamsWebhook, dashboardUrl, canonicalTableName, username);
      }
    } catch (blockError) {
